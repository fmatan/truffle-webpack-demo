...
/*
// Global constraints implemented for DAOstack V0.1:
// 10M tokens in Total
// Changable only *once a year*, beginning only after first two years, and only with a wide consensus of 75% of voting stakeholders (i.e. vote=tokens).
// Token value in PCO with (at least) 33% discount w.r.t. ICO.
...
contract DAOstackGlobalConstraint is GlobalConstraintInterface {
...
/*
    ...
    function post( address _scheme, bytes _param ) returns(bool) {
      ...
      function capTotalSupply() constant returns(bool){
        // cap the total supply of tokens at 10M
...
contract GlobalConstraintChange {
    Controller                 controller;
    uint                       submissionFee;
    SimpleVoteInterface public simpleVote;

    bytes32 globalSchemeDescription;

    mapping(bytes32=>globalSchemeDescription) contributions;
